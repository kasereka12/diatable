...
export default function MessagingPanel({ heightClass = 'h-[calc(100vh-240px)] min-h-[500px]' }: { heightClass?: string }) {
    ...
    async function load() {
      ...
      if (convs.length > 0) {
        ...
        if (unreadData) {
          ...
          ;(unreadData as any[]).forEach((msg: any) => {
      ...
      if (convId && convs) {
        const target = convs.find((c: any) => c.id === convId)
        if (target) {
  ...
  const getConvInitials = (conv: any) => {
    const name = getConvName(conv)
    return name.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2)
  }

  return (
    <div className={`bg-white rounded-2xl shadow-sm border border-black/[0.05] overflow-hidden ${heightClass}`}>
