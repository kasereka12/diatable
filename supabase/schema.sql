...
-- ── 21b. restaurant_listings (vue) ────────────────────
-- Pré-joint restaurant + plan actuel + stats d'avis, avec un plan_rank
-- numérique, pour que la page /restaurants pagine et trie entièrement côté
-- Postgres (.order() + .range()) au lieu de tout charger et trier en JS.
-- Vue "plain" = exécutée avec les droits du propriétaire, pas ceux de
-- l'appelant : le filtre is_active doit donc être répété ici explicitement.
-- restaurants a ses propres colonnes rating/reviews héritées (toujours à
-- 0/null, remplacées par vendeurs_stats) — r.* entrerait en collision avec
-- le rating/reviews calculés ci-dessous, d'où la liste explicite.
...
-- ── 21c. WALLET — solde vendeur + retraits ────────────
-- Modèle "comptabilité bancaire" : chaque commande payée par carte crédite
-- un solde interne par restaurant (sous-total moins une commission
-- plateforme réglable dans platform_settings.commission_pct). Le vendeur
-- demande un retrait contre ce solde ; l'admin vire manuellement puis marque
-- le retrait "payé". Les commandes en espèces ne créditent pas le solde (le
-- restaurant encaisse déjà le cash directement).
...
-- Vue solde (security_invoker : RLS de l'appelant s'applique). available_balance
-- réserve les retraits en attente/approuvés pour empêcher un double retrait.
...
left join (
  ...
  from wallet_transactions where type = 'credit' group by restaurant_id
...
left join (
  ...
  from withdrawals group by restaurant_id
...
begin
  select available_balance into avail from restaurant_wallets where restaurant_id = new.restaurant_id;
